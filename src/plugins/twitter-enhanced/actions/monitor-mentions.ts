...
export const monitorMentionsAction: Action = {
  ...
  validate: async (runtime: IAgentRuntime, message: Memory) => {
    ...
    return (
      content.includes('mention') || content.includes('notification') || content.includes('monitor')
    );
  },
        ...
        const statusText =
          ...
          (history.length > 0
            ? history
                .map(
                  (h) =>
                    `• ${h.type}: @${h.author} - ${new Date(h.timestamp).toLocaleTimeString()}`,
                )
                .join('\n')
            : '• No recent activity');
  ...
  examples: [
    ...
    [
      ...
      },
      {
        name: 'assistant',
        content: {
          text: "I'll start monitoring your Twitter mentions and notifications in real-time!",
        },
      },
    ],
  ],
