...
export const submitEngagementAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      else if (text.includes('commented') || text.includes('comment') || text.includes('replied'))
      ...
      if (result.success) {
        ...
        if (callback) {
          ...
          const rankText =
            rankChange > 0
              ? `📈 +${rankChange} rank positions!`
              : rankChange < 0
                ? `📉 ${Math.abs(rankChange)} rank positions`
                : '📊 Rank maintained';
      ...
      if (callback) {
        callback({
          text:
            '⚠️ **ENGAGEMENT SUBMISSION FAILED!** ⚠️\n\n' +
