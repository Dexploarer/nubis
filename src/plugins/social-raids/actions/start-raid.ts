...
export const startRaidAction: Action = {
  name: 'START_RAID',
  similes: ['START_TWITTER_RAID', 'INITIATE_RAID', 'BEGIN_RAID', 'LAUNCH_RAID', 'CREATE_RAID'],
  validate: async (runtime: IAgentRuntime, message: Memory) => {
    const text = message.content?.text?.toLowerCase() || '';

    // Check if message contains raid-related keywords and a Twitter URL
    const hasRaidKeywords =
  ...
  handler: async (
    ...
    try {
      ...
      if (result.success) {
        ...
        // Post raid status to Twitter (non-blocking)
        try {
          const twitterService = runtime.getService('TWITTER_RAID_SERVICE');
          if (twitterService && typeof (twitterService as any).postRaidStatus === 'function') {
            await (twitterService as any).postRaidStatus({
          ...
          elizaLogger.error('Failed to post raid status to Twitter:', error);
          // Non-blocking - continue with raid even if Twitter posting fails
        }
