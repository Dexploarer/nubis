...
export class KnowledgeOptimizationService extends Service {
  ...
  capabilityDescription =
    'Optimizes ElizaOS Knowledge Plugin usage and manages community-specific documentation';
  ...
  private categorizeDocument(filePath: string, content: string): string {
    ...
    if (
      ...
      contentLower.includes('nubi') ||
      contentLower.includes('anubis')
    )
      return 'character-lore';
    if (pathLower.includes('architecture') || contentLower.includes('elizaos'))
      return 'technical-architecture';
    if (pathLower.includes('memory') || contentLower.includes('memory system'))
    ...
    if (pathLower.includes('twitter') || contentLower.includes('telegram'))
  ...
  private extractTags(content: string): string[] {
    ...
    const techMatches = content.match(
      /\b(elizaos|nubi|anubis|solana|web3|ai|agent|plugin|memory|knowledge|community|raid|twitter|telegram|supabase|database)\b/gi,
    );
    if (techMatches) {
  ...
  private groupDocumentsByCategory(): KnowledgeCategory[] {
    ...
    for (const [categoryName, documents] of categoryMap) {
      const avgPriority =
        documents.reduce((sum, doc) => sum + doc.metadata.priority, 0) / documents.length;
  ...
  private async generateOptimizationReport(): Promise<void> {
    ...
    for (const category of this.categories) {
      ...
      for (const doc of category.documents.slice(0, 5)) {
  ...
  private generateRecommendations(): string {
    ...
    if (!categories.includes('character-lore')) {
      recommendations +=
        '- **Character Development Gap**: Consider adding more character lore documentation\n';
    }

    if (!categories.includes('community')) {
      recommendations +=
        '- **Community Gap**: Add community interaction guidelines and management docs\n';
    }
...
Object.defineProperty(KnowledgeOptimizationService, 'name', {
  value: KnowledgeOptimizationService.serviceType,
});
