...
export const ParticipationConsistencyEvaluator: Evaluator = {
  ...
  validate: async (_runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
    ...
    const history =
      (message.content as any)?.engagementHistory ||
      (message.content as any)?.userEngagements ||
      [];
  ...
  handler: async (_runtime: IAgentRuntime, message: Memory): Promise<void> => {
    try {
      ...
      for (let i = 1; i < sorted.length; i++) {
        intervals.push(
          ((sorted[i].ts as Date).getTime() - (sorted[i - 1].ts as Date).getTime()) / 1000,
        ); // seconds
      }
      ...
      elizaLogger.debug(
        `Participation consistency score: ${score.toFixed(2)} (cv=${cv.toFixed(2)})`,
      );
      return;
    } catch (err) {
