...
export class EngagementQualityEvaluator implements Evaluator {
        ...
        await runtime.createMemory?.(
          {
            ...
            content: {
              ...
              suspiciousPatterns: [],
              recommendations: [
                'Use a valid engagement type: like, retweet, quote, comment, verify',
              ],
            },
          } as any,
          'engagement_evaluations',
  ...
  detectSuspiciousPatterns(
    engagements: Array<{ actionType?: string; timestamp?: Date | number | string }>,
  ): string[] {
    ...
    const likeCount = engagements.filter(
      (e) => (e.actionType || '').toLowerCase() === 'like',
    ).length;
...
export const EngagementQualityEvaluatorExamples = [
  {
    ...
    outcome:
      'High quality engagement detected - thoughtful commentary adds significant value (Score: 0.85)',
  },
  {
    prompt: "Evaluate the engagement quality of the user's message.",
    messages: [
    ...
    outcome:
      'Exceptional quality engagement - comprehensive analysis with community focus (Score: 0.92)',
  },
];
