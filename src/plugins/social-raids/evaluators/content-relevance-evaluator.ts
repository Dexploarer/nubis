...
export const ContentRelevanceEvaluator: Evaluator = {
  ...
  validate: async (_runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
    ...
    const target =
      (message.content as any)?.targetContent || (message.content as any)?.referenceText || '';
    // If neither text nor target context is present, not valid
    if (!text && !target) return false;
    const triggers = ['comment', 'quote', 'reply', 'retweet', 'analysis', 'discuss', 'engage'];
    return !!target || triggers.some((t) => text.includes(t));
  },
  ...
  examples: [
      ...
      messages: [
        {
          name: '{{user1}}',
          content: {
            text: 'Quoted with analysis about scalability and community-led growth.',
            targetContent: 'This thread discusses community-led scalable growth models.',
          },
        },
      ],
    ...
    },
    {
      ...
      messages: [{ name: '{{user1}}', content: { text: 'Awesome! Love it!' } }],
