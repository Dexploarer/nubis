...
export const SpamScoreEvaluator: Evaluator = {
  ...
  handler: async (_runtime: IAgentRuntime, message: Memory): Promise<void> => {
    try {
      ...
      const capsRatio = textRaw
        ? textRaw.replace(/[^A-Z]/g, '').length / Math.max(1, textRaw.length)
        : 0;
  ...
  examples: [
    {
      ...
      messages: [
        {
          name: '{{user1}}',
          content: { text: 'CLICK HERE!!! Free giveaway, follow me and buy now!' },
        },
      ],
    ...
    },
    {
      ...
      messages: [
        {
          name: '{{user1}}',
          content: { text: 'I commented thoughtfully on the tweet with my perspective.' },
        },
      ],
